  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const schedulesResult = (await findMany("schedule", {
          where: scheduleQueryWhere,
          include: {
            ship: true,
          },
        })) as {
          data: Prisma.ScheduleGetPayload<{ include: { ship: true } }>[] | null;
          error?: string | null;
        };

        if (schedulesResult.error || !schedulesResult.data) {
          setError(schedulesResult.error || "Could not retrieve schedule data.");
          setTotalDaysSchedule([]);
          return;
        }
